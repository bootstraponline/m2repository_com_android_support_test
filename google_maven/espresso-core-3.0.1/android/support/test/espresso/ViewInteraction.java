...
public final class ViewInteraction {
  ...
  private void desugaredPerform(final SingleExecutionViewAction va) {
    ...
    if (!remoteInteraction.isRemoteProcess()) {
      ...
      interactions.add(
          remoteInteraction.runPerformRemotely(
              rootMatcherRef.get(),
              viewMatcher,
  ...
  public ViewInteraction check(final ViewAssertion viewAssert) {
        ...
        new Callable<Void>() {
          ...
          public Void call() {
            ...
            Log.i(
                TAG,
                String.format(
                    "Checking '%s' assertion on view %s", viewAssert, viewMatcher));
    ...
    if (!remoteInteraction.isRemoteProcess()) {
      // Only the original process should submit remote interactionsList;
      interactions.add(
          remoteInteraction.runCheckRemotely(
              rootMatcherRef.get(),
              viewMatcher,
  ...
  private void waitForAndHandleInteractionResults(List<ListenableFuture<Void>> interactions) {
    InteractionResultsHandler interactionResultsHandler =
        new InteractionResultsHandler(interactions);

    try {
      // Blocking call
      interactionResultsHandler.get();
    } catch (InterruptedException ie) {
      throw new IllegalStateException(
          "Interrupted while waiting for interaction response.", ie.getCause());
