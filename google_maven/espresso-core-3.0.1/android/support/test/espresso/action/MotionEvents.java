...
public final class MotionEvents {
  ...
  public static DownResultHolder sendDown(
    ...
    return sendDown(uiController, coordinates, precision, InputDevice.SOURCE_UNKNOWN,
    ...
    for (int retry = 0; retry < MAX_CLICK_ATTEMPTS; retry++) {
      ...
      try {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
          motionEvent = downPressGingerBread(downTime, coordinates, precision);
        } else {
          motionEvent = downPressICS(downTime, coordinates, precision, inputDevice, buttonState);
        }
  ...
  public static boolean sendUp(
    ...
    try {
      // Up press.
      if (Build.VERSION.SDK_INT < Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
        motionEvent = upPressGingerBread(downEvent, coordinates);
      } else {
        motionEvent = upPressICS(downEvent, coordinates);
      }
      boolean injectEventSucceeded = uiController.injectMotionEvent(motionEvent);
  ...
  private static MotionEvent downPressGingerBread(long downTime, float[] coordinates,
      float[] precision) {
  ...
  private static MotionEvent downPressICS(long downTime, float[] coordinates, float[] precision,
      int inputDevice, int buttonState) {
  ...
  private static MotionEvent upPressGingerBread(MotionEvent downEvent, float[] coordinates) {
    return MotionEvent.obtain(downEvent.getDownTime(),
        SystemClock.uptimeMillis(),
