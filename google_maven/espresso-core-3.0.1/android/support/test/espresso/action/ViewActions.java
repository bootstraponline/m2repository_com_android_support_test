...
public final class ViewActions {
   ...
   * <p>
   * <b>Note:</b> Not supported by API < 14. An {@link UnsupportedOperationException} will be thrown
   * if called on API < 14. For API < 14, call {@link #click()} instead.<br>
   * <br>
   * View constraints:
   * <ul>
   * <li>must be displayed on screen
   * <ul>
   *
   * @param inputDevice source input device of the click. Example:
   *  {@link InputDevice#SOURCE_MOUSE}
   * @param buttonState buttonState associated with the click. Example:
   *  {@link MotionEvent#BUTTON_PRIMARY}
   */
  ...
  /**
   * Same as {@code click(int inputDevice, int buttonState)}, but uses
   * {@link InputDevice#SOURCE_UNKNOWN} as the inputDevice and {@link MotionEvent#BUTTON_PRIMARY}
   * as the buttonState.
   */
  ...
  public static ViewAction click(ViewAction rollbackAction) {
    ...
    return actionWithAssertions(
        new GeneralClickAction(Tap.SINGLE, GeneralLocation.CENTER, Press.FINGER,
            InputDevice.SOURCE_UNKNOWN, MotionEvent.BUTTON_PRIMARY, rollbackAction));
  }
