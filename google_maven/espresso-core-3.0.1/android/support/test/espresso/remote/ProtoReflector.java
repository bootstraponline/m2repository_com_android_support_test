...
import static android.support.test.espresso.remote.ProtoUtils.capitalizeFirstChar;
...
final class ProtoReflector {
   ...
   *
   * @param methodSuffix the method suffix to append "get" to create the full method
   *     name. For instance, if the proto method is called {@code getSomeSuffix()} the method
   *     prefix needs to be "SomeSuffix".
  ...
  private <T> T invokeMethod(MessageLite messageLite, String methodNameTpl,
      String methodSuffix, Class<T> clazz) {
    return clazz.cast(new MethodInvocation(protoType, messageLite,
        String.format(methodNameTpl, capitalizeFirstChar(methodSuffix))).invokeDeclaredMethod());
  }
}
