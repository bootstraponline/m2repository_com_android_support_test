...
public final class RemoteDescriptor {
  ...
  public static final class Builder {
     ...
     * constructor.
     *
     * <p>By default the {@code instanceType} is used as remote message constructor. Only set
     * custom remote constructor types, when the remote message constructor takes a superclass or
    ...
    public RemoteDescriptor build() {
      ...
      try {
        if (remoteType.isAssignableFrom(Class.forName(GENERIC_REMOTE_MESSAGE_CLS))) {
          ...
          throw new IllegalStateException(String.format("Could not load class for name: %s",
              GENERIC_REMOTE_MESSAGE_CLS), cnfe);
      }
      ...
      if (null == protoParser) {
        ...
        protoParser = (Parser<?>) new MethodInvocation(protoType, null, "parser")
            .invokeMethod();
        checkState(protoParser != null,
