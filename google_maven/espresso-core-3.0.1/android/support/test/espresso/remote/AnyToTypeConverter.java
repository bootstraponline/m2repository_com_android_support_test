...
import android.support.test.espresso.remote.EspressoRemoteMessage.From;
...
final class AnyToTypeConverter<T> implements Converter<Any, T> {
  ...
  public T convert(@NonNull Any any) {
    ...
    try {
      ...
      throw new RemoteProtocolException(
          "Cannot unwrap target type instance from any proto: ", iae);
    } catch (RuntimeException re) {
      throw new RemoteProtocolException(String.format("Something went wrong during any conversion "
          + "for type url: %s", any.getTypeUrl()),
          re);
    }
    return targetType;
  }
  ...
  private EspressoRemoteMessage.From<T, MessageLite> createRemoteMessageFromField(Field fromField)
    ...
    try {
      ...
      throw new RemoteProtocolException("Cannot unwrap target type from any proto: Cannot cast"
  ...
  private Field accessFromField(Class<?> remoteClass) throws IllegalAccessException {
    ...
    try {
      ...
      throw new RemoteProtocolException("Cannot unwrap target type from any proto: "
          ...
          + "fromViewMatcherField exists!", nsfe);
    }
  ...
  private <T extends MessageLite> T anyToProto(
      Any any, Class<T> expectedType, Parser<?> parser) throws InvalidProtocolBufferException {
    ...
    if (!expectedType.isInstance(proto)) {
      RemoteProtocolException rpe = new RemoteProtocolException(
          String.format("Got type: %s, but expected type: %s instead", proto.getClass().getName(),
  ...
  private <T extends MessageLite> T anyToProto(Any any) {
    ...
    try {
      ...
      throw new RemoteProtocolException("Invalid Protocol buffer for any type url: "
          + any.getTypeUrl(), ipbe);
    }
  }
}
