...
import static android.support.test.espresso.remote.InteractionResponse.Status.Ok;
...
import android.support.test.espresso.proto.UiInteraction.Error;
import android.support.test.espresso.proto.UiInteraction.InteractionResultProto;
import android.support.test.espresso.remote.EspressoRemoteMessage.To;
...
public final class InteractionResponse implements To<MessageLite> {
            ...
            public InteractionResponse fromProto(InteractionResultProto interactionResultProto) {
              ...
              if (errorMsg != null) {
                ...
                if (RemoteError.isWellKnownError(errorCode)) {
                  remoteError = new RemoteError(errorCode,
                      RemoteError.getWellKnownFormattedErrorDescription(errorCode,
                          errorDescription));
                } else {
  ...
  /**
   * The status of this interaction response. Can either be {@link Status#Ok} or
   * {@link Status#Error}
   */
  public enum Status {
    Ok,
  ...
  public static final class RemoteError {
    ...
    private static final RemoteError REMOTE_ESPRESSO_ERROR =
        new RemoteError(REMOTE_ESPRESSO_ERROR_CODE, "The following remote Espresso exception with "
            + "error code [%s] occurred:\n%s");
    private static final RemoteError REMOTE_PROTOCOL_ERROR =
        new RemoteError(REMOTE_PROTOCOL_ERROR_CODE, "The following remote protocol Espresso "
    ...
    private static String formatDescription(String description, int errorCode,
     ...
     * <p>
     * This method allows to provide a formatted error message using the specified format error
  ...
  public static class Builder {
    ...
    /**
     * Set the result proto as a byte array. This byte array will be parsed into an
     * {@link InteractionResultProto}. Providing an invalid byte byte array will result in a
     * {@link RemoteProtocolException} when the {@link #build()} method is called!
     * @param protoByteArray the proto byte array to set
     * @return fluent interface {@link Builder}
     */
    public Builder setResultProto(@NonNull byte[] protoByteArray) {
      this.interactionResultProtoByteArray = checkNotNull(protoByteArray, "protoByteArray cannot be"
          + "null!");
      return this;
    }
     ...
     *
     * @throws RemoteProtocolException when conflicting properties are set. You can either set
     * an {@link Status} with an optional {@link RemoteError} error in case that the status is
     * {@link Status#Error} or set the proto byte array not both. Setting both values would result
     * in an override, therefore setting both properties will result in an exception.
     *
    ...
    public InteractionResponse build() {
      if (status != null || remoteError != null) {
        if (interactionResultProtoByteArray != null) {
          throw new RemoteProtocolException("Instances can either be create from an status and "
