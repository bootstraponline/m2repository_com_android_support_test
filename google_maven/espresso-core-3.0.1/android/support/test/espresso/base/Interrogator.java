...
final class Interrogator {
  ...
  /**
   * Informed of the state of the queue and controls whether to continue interrogation or
   * quit.
   */
        ...
        if (stillInterested) {
          ...
          if (newIdentity != threadIdentity) {
            Log.wtf(TAG, "Thread identity changed from 0x"
                + Long.toHexString(threadIdentity) + " to 0x"
                + Long.toHexString(newIdentity) +  " while dispatching to "
                + m.getTarget().getClass().getName() + " "
                + m.getCallback() + " what=" + m.what);
          }
          recycle(m);
        }
      }
   ...
   * barrier.
   *
   * This method can be called from any thread. It has limitations though - if a task is currently
   * being executed in the interrogation loop, you will not know about it. If the Looper is quitting
   * you will not know about it. You can only see the state of the message queue - which is seperate
   * from the state of the overall loop.
   *
  ...
  static <R> R peekAtQueueState(MessageQueue q, QueueInterrogationHandler<R> handler) {
    ...
    checkState(!interrogateQueueState(q, handler),
        "It is expected that %s would stop interrogation after a single peak at the queue.",
        handler);
    return handler.get();
  }

  private static boolean interrogateQueueState(MessageQueue q,
      QueueInterrogationHandler<?> handler) {
    synchronized (q) {
      final Message head;
      try {
      ...
      if (Log.isLoggable(TAG, Log.DEBUG)) {
        Log.d(TAG, "headWhen: " + headWhen
            + " nowFuz: " + nowFuz
            + " due long: " + (nowFuz < headWhen));
      }
      if (nowFuz > headWhen) {
