...
public final class Web {
  static {
    UsageTrackerRegistry.getInstance().trackUsage("Espresso-Web", AtslVersions.ESPRESSO_VERSION);
  }
  ...
  public static class WebInteraction<R> {
    ...
    private WebInteraction(Matcher<View> viewMatcher, R result, WindowReference window,
        ElementReference element, boolean brandNew, Timeout timeout) {
     ...
     *
     * This is usually necessary when a prior action (for example a click) introduces a navigation
     * that invalidates the ElementReference and WindowReference pointers.
     */
     ...
     *
     * All WebView interactions are done via Javascript - therefore the WebView we are working on
     * must support Javascript evaluation.
     *
     * Enabling Javascript may cause the WebView under test to be reloaded. This is necessary to
    ...
    public WebInteraction<R> forceJavascriptEnabled() {
      onView(viewMatcher)
          .perform(new EnableJavascriptAction());
      return this;
    }
     ...
     *
     * Javascript evaluation is performed asynchronously on the WebKit/Chromium thread. By default
     * we wait a short while for the result to be delivered back to the test.
     */
    ...
    public WebInteraction<R> withTimeout(long amount, TimeUnit unit) {
      return new WebInteraction<R>(viewMatcher, result, window, element, brandNew,
          new Timeout(amount, unit, true));
    }

    /**
     * Causes this WebInteraction to have it's javascript evaluated in a particular DOM window.
     *
     * By default Javascript may be evaluated in the main window. However in an application which
     ...
     * Causes this WebInteraction to have it's javascript evaluated in a particular DOM window.
     *
     * This method accepts an Atom which will be evaluated in the main window to choose a particular
     * DOM window for further interactions. This method will block until the the provided Atom
     * returns with a result.
     */
    public WebInteraction<R> inWindow(Atom<WindowReference> windowPicker) {
      return new WebInteraction<R>(viewMatcher, result,
          doEval(windowPicker, null, null), element, brandNew, timeout);
    }
     ...
     * {@see Atom#getArguments}
     *
     * This method accepts an Atom<ElementReference> which it will evaluate on the current context's
     * Window. This method blocks until the evaluation completes.
     *
     * Calling this method resets any previously selected ElementReference.
     *
    ...
    public WebInteraction<R> withElement(Atom<ElementReference> elementPicker) {
      return new WebInteraction<R>(viewMatcher, result, window,
          doEval(elementPicker, window, null), brandNew, timeout);
    }

    /**
     * Allows for contextually evaluating this WebInteraction with the selected element.
     *
     * Specifically the elementPicker atom will be evaluated with the CURRENTLY selected element
    ...
    public WebInteraction<R> withContextualElement(Atom<ElementReference> elementPicker) {
      return new WebInteraction<R>(viewMatcher, result, window,
          doEval(elementPicker, window, element), brandNew, timeout);
    }
     ...
     * References).
     *
     * This method blocks until the Atom returns. The result of the Atom's evaluation is used to
     ...
     *
     * The WebAssertion's atom is evaluated, after it's evaluation completes, the WebAssertion is
    ...
    private <E> E doEval(Atom<E> atom, WindowReference window, ElementReference elem) {
      ...
      try {
        ...
        onView(viewMatcher)
            .perform(new ExceptionPropagator(ee.getCause()));
        return null;  // always throws.
      } catch (InterruptedException ie) {
        onView(viewMatcher)
            .perform(new ExceptionPropagator(ie));
        return null;  // always throws.
      } catch (TimeoutException te) {
        onView(viewMatcher)
            .perform(new ExceptionPropagator(te));
        return null;  // always throws.
      } catch (RuntimeException re) {
        onView(viewMatcher)
            .perform(new ExceptionPropagator(re));
        return null;  // always throws.
      }
    }
