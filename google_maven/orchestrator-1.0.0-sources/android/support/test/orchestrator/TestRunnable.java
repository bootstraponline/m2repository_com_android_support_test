...
import android.support.test.services.shellexecutor.ShellCommandClient.ClientNotConnected;
...
public class TestRunnable implements Runnable {
  ...
  private static final String TARGET_INSTRUMENTATION_KEY = "targetInstrumentation";
  private static final String LIST_TESTS_ARGUMENT = "listTestsForOrchestrator";

  static final String ISOLATED_KEY = "isolated";
  ...
  private String getTargetInstrumentation() {
    return mArguments.getString(TARGET_INSTRUMENTATION_KEY);
  }
  ...
  private Bundle getTargetInstrumentationArguments() {
    ...
    targetArgs.remove(TARGET_INSTRUMENTATION_KEY);
    targetArgs.remove(ISOLATED_KEY);

    if (mCollectTests) {
      targetArgs.putString(LIST_TESTS_ARGUMENT, "true");
    } else {
    ...
    if (mTest != null) {
      targetArgs.putString("class", mTest);
    }
